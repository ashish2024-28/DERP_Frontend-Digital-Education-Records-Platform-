...
//   // ── UPDATE PASSWORD ──
//   // Backend endpoint: PUT /{domain}/domainAdmin/forgot_update_password?newPassword=
...
//         setActionErr(text || "Failed to add.");
//       }
...
//       } else {
//         const text = await res.text();
//         setActionErr(text || "Update failed.");
//       }
//     } catch (err) {
//       setActionErr("Network error: " + err.message);
//     }
...
//       const text = await res.text();
//       if (res.ok) {
//         setActionMsg(text || "Deleted successfully!");
...
//   // ── EXCEL BULK UPLOAD ──
//   // Backend endpoints:
//   //   POST /{domain}/domainAdmin/upload_students (file)
//   //   POST /{domain}/domainAdmin/upload_faculty  (file)
//   //   POST /{domain}/domainAdmin/upload_subAdmin (file)
//   const handleExcelUpload = async () => {
//     clearFeedback();
//     if (!uploadFile) { setActionErr("Please select an Excel file first."); return; }
//     const endpointMap = {
//       student:  "upload_students",
//       faculty:  "upload_faculty",
//       subAdmin: "upload_subAdmin",
//     };
//     try {
//       setUploading(true);
//       const formData = new FormData();
...
//     : activeTab === "faculty" ? facultyFields
//     : subAdminFields;
...
//               padding: "9px", borderRadius: 6, border: "1px solid #2563eb",
...
//               <Link className="main-content-Link" to={"all-students"}>
//                 <div className="card">All Students: {totalStudent}</div>
//               </Link>
//               <Link className="main-content-Link" to={"all-faculty"}>
//                 <div className="card">All Faculty: {totalFaculty}</div>
//               </Link>
//               <Link className="main-content-Link" to={"all-subAdmin"}>
//                 <div className="card">All SubAdmin: {totalSubAdmin}</div>
//               </Link>
...
//                 style={{ fontWeight: activeTab === "student" ? 700 : 400, borderBottom: activeTab === "student" ? "2px solid #2563eb" : "none" }}
//               >
//                 Students ({totalStudent})
//               </button>
//               <button
//                 onClick={() => setActiveTab("faculty")}
//                 style={{ fontWeight: activeTab === "faculty" ? 700 : 400, borderBottom: activeTab === "faculty" ? "2px solid #2563eb" : "none" }}
//               >
//                 Faculty ({totalFaculty})
//               </button>
//               <button
//                 onClick={() => setActiveTab("subAdmin")}
//                 style={{ fontWeight: activeTab === "subAdmin" ? 700 : 400, borderBottom: activeTab === "subAdmin" ? "2px solid #2563eb" : "none" }}
//               >
...
//                 {/* Add Button */}
//                 <button
//                   onClick={() => { setAddForm({}); clearFeedback(); setShowAddModal(true); }}
//                   style={{
...
//                 {/* Excel Upload Button */}
//                 <button
//                   onClick={() => { clearFeedback(); setUploadFile(null); setShowUploadModal(true); }}
//                   style={{
//                     padding: "8px 16px", borderRadius: 6, border: "none",
...
//                           style={{
//                             marginRight: 6, marginBottom: 4, padding: "4px 12px", borderRadius: 5,
...
//                     <tr>
//                       <td colSpan={20} style={{ textAlign: "center", color: "#9ca3af", padding: 24 }}>
...
//             <img
//               src={profilePicPreview || (admin.profilePic ? `${API_BASE}/${admin.profilePic}` : "/default.png")}
//               alt="Preview"
...
//               {activeTab === "student" && "Columns: name, email, password, mobileNumber, rollNumber, course, branch, batch, fatherName, fatherMobNo"}
//               {activeTab === "faculty" && "Columns: name, email, password, mobileNumber, facultyId, course, teachingBatch"}
//               {activeTab === "subAdmin" && "Columns: name, email, password, mobileNumber, subAdminId, course"}
//             </p>
//           </div>
...
//           <button
//             style={{ ...submitBtnStyle, background: uploading ? "#9ca3af" : "#0891b2" }}
...
// // // ─────────────────────────────────────────────
// // // Reusable Modal wrapper (added – no old code removed)
// // // ─────────────────────────────────────────────
// // function Modal({ title, onClose, children }) {
// //   return (
// //     <div style={modalStyles.overlay}>
...
// //   // ─────────────────────────────────────────────
// //   // NEW: ADD (Student / Faculty / SubAdmin)
// //   // ─────────────────────────────────────────────
// //   const handleAdd = async () => {
// //     clearFeedback();
// //     const endpointMap = { student: "add_student", faculty: "add_faculty", subAdmin: "add_subAdmin" };
...
// //   // ─────────────────────────────────────────────
// //   // NEW: EDIT (Student / Faculty / SubAdmin)
// //   // ─────────────────────────────────────────────
// //   const openEditModal = (item) => {
// //     setSelectedItem(item);
// //     setEditForm({ ...item });
...
// //   const handleEdit = async () => {
// //     clearFeedback();
// //     // FIX: endpoint names updated to match new backend routes
// //     const endpointMap = {
// //       student: "update_student_profile",
// //       faculty: "update_faculty",       // was: update_faculty_by_facultyId
// //       subAdmin: "update_subadmin",      // was: update_subadmin_by_subadminId
// //     };
// //     try {
// //       const res = await fetch(`${API_BASE}/${domain}/domainAdmin/${endpointMap[activeTab]}`, {
// //         method: "PUT",
...
// //   // ─────────────────────────────────────────────
// //   // NEW: DELETE (Student / Faculty / SubAdmin)
// //   // ─────────────────────────────────────────────
// //   const openDeleteModal = (item) => {
// //     setSelectedItem(item);
// //     clearFeedback();
...
// //     // FIX: endpoint names updated to match new backend routes
// //     const endpointMap = {
// //       student: `delete_student?email=${encodeURIComponent(selectedItem.email)}`,   // was: delete_student_by_email
// //       faculty: `delete_faculty?email=${encodeURIComponent(selectedItem.email)}`,   // was: delete_faculty_by_facultyId
// //       subAdmin: `delete_subadmin?email=${encodeURIComponent(selectedItem.email)}`,  // was: delete_subadmin_by_subadminId
// //     };
// //     try {
// //       const res = await fetch(`${API_BASE}/${domain}/domainAdmin/${endpointMap[activeTab]}`, {
// //         method: "DELETE",
...
// //           {/* NEW: Change Password button in sidebar */}
// //           <button
// //             style={{
// //               width: "calc(100% - 32px)", margin: "0 16px 10px",
// //               padding: "9px", borderRadius: 6, border: "1px solid #2563eb",
...
// //         {/* NEW: Global feedback banner */}
// //         {actionMsg && (
// //           <div style={{
// //             background: "#d1fae5", border: "1px solid #6ee7b7",
// //             color: "#065f46", borderRadius: 8, padding: "10px 16px",
...
// //             color: "#991b1b", borderRadius: 8, padding: "10px 16px",
// //             marginBottom: 12, display: "flex", justifyContent: "space-between",
// //           }}>
// //             ❌ {actionErr}
// //             <span style={{ cursor: "pointer", fontWeight: 700 }} onClick={() => setActionErr("")}>✕</span>
...
// //               {/* NEW: Row with search + Add button */}
...
// //                           onClick={() => openEditModal(item)}
// //                           style={{
// //                             marginRight: 6, marginBottom: 5, padding: "4px 12px", borderRadius: 5,
// //                             border: "none", background: "#2563eb", color: "#fff",
// //                             cursor: "pointer", fontWeight: 600, fontSize: 13,
// //                           }}
// //                         >
...
// //           <div style={{ textAlign: "center", marginBottom: 16 }}>
// //             <img
// //               src={profilePicPreview || (admin.profilePic) || "/default.png"}
// //               alt="Preview"
// //               style={{ width: 110, height: 110, borderRadius: "50%", objectFit: "cover", border: "3px solid #e5e7eb" }}
// //             />
// //           </div>
          ...
          <>
            ...
            <div className="shadow rounded-xl p-6">
              ...
              <table className="table-wrapper">
                <thead>
                  <tr>
                    <th>S.No</th><th>Name</th><th>Email</th><th>Mobile</th>
                    {activeTab === "student" && <><th>Roll No</th><th>Branch</th><th>Batch</th><th>Father Name</th><th>Father Mob</th></>}
                    {activeTab === "faculty"  && <><th>Faculty ID</th><th>Teaching Batch</th></>}
                    {activeTab === "subAdmin" && <th>SubAdmin ID</th>}
                    <th>Course</th><th>Created At</th><th>Last Login</th>
                    <th>
                      Password
                ...
                <tbody>
                  {filteredData.map((item, index) => (
                    <tr key={index}>
                      <td>{index + 1}</td><td>{item.name}</td><td>{item.email}</td><td>{item.mobileNumber}</td>
                      {activeTab === "student" && <><td>{item.rollNumber}</td><td>{item.branch}</td><td>{item.batch}</td><td>{item.fatherName}</td><td>{item.fatherMobNo}</td></>}
                      {activeTab === "faculty"  && <><td>{item.facultyId}</td><td>{item.teachingBatch}</td></>}
